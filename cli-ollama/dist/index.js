...
function parseConfigInitArgs(args) {
...
function applyOverrides(config, overrides) {
    ...
    if (typeof overrides.agents === 'number') {
        const modelFallback = overrides.model ?? updated.agents[0]?.model ?? 'llama3.1:8b';
        updated.agents = resizeAgents(updated.agents, overrides.agents, modelFallback);
    }
    return updated;
}
function resizeAgents(agents, count, modelFallback) {
